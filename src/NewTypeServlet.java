...
/**
 * Servlet implementation class NewTypeName
 */
@WebServlet("/NewTypeServlet")
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			try (Connection conn = SqlConnection.getConnection()) {
				System.out.println("Connection to WareHouse.Type succesfull!\n");
				String name = request.getParameter("name");
				int id = SqlConnection.InsertType(name);
				response.setContentType("text/html");
				PrintWriter writer = response.getWriter();
				try {
					writer.println("<link rel=\"stylesheet\" type=\"text/css\" href=\"css/style.css\">");
					writer.println("<div class=\"menu\"><a href=\"index.html\"> <button class=\"button\"><span>"+id+"</span></button>");
				} finally {
					writer.close();
				}
			}
